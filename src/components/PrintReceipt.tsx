  ...
  return (
    ...
    >
      ...
      <div className="border-b-2 border-dashed border-black pb-4 mb-4">
        ...
        <h2 className="text-[10px] font-semibold tracking-tight uppercase">Hotel & Convention Center</h2>
      ...
      {/* Luggage ID Tag */}
      <div className="mb-4">
        <div className="text-[9px] uppercase tracking-wider text-slate-500">LUGGAGE BAGGAGE TICKET</div>
        <div className="text-base font-bold tracking-wider my-1 border border-black py-1.5 bg-black/5 rounded font-mono">
          {item.id}
        </div>
        <div className="text-[9px] text-slate-600">Status: {item.status.toUpperCase()}</div>
      </div>
      ...
      <div className="flex justify-center my-4">
        {item.qrCode ? (
          <img 
            src={item.qrCode} 
            alt="Baggage QR Code" 
            className="w-40 h-40 object-contain p-1 border border-slate-200"
      ...
      <div className="text-left text-[10px] space-y-1.5 border-t-2 border-b-2 border-dashed border-black py-4 my-4">
        ...
        {item.remark && (
          <div className="border-t border-slate-100 pt-1.5 mt-1.5">
            <span className="text-slate-500 block text-[9px] font-bold uppercase">REMARK:</span>
            <span className="italic block mt-0.5 text-[9px] text-slate-700 font-sans">&quot;{item.remark}&quot;</span>
          </div>
        )}
      </div>

      {/* Terms and Policies */}
      <div className="space-y-2 mt-4 text-[8px] text-slate-600 leading-normal text-left">
        <div className="flex items-start gap-1">
          <ShieldCheck className="w-3.5 h-3.5 shrink-0 text-black inline mt-0.5" />
        ...
        <div className="flex items-start gap-1">
          <Info className="w-3.5 h-3.5 shrink-0 text-black inline mt-0.5" />
          <span>ASTON Cirebon tidak bertanggung jawab atas kerusakan atau kehilangan barang di luar syarat & ketentuan hotel.</span>
        </div>
      </div>

      {/* Footer thank you */}
      <div className="mt-6 pt-3 border-t border-slate-100 text-[9px] italic uppercase tracking-wider">
